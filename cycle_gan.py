...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, device, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = mse_loss(D_X(images_X), torch.ones(len(images_X)).to(device))
        D_Y_loss = mse_loss(D_Y(images_Y), torch.ones(len(images_Y)).to(device)) 
        ...
        D_X_loss = mse_loss(D_X(fake_X), torch.zeros(len(fake_X)).to(device))

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = mse_loss(D_Y(fake_Y), torch.zeros(len(fake_Y)).to(device))
        ...
        g_loss = mse_loss(D_X(fake_X), torch.ones(len(fake_X)).to(device)) 
        ...
        g_loss = mse_loss(D_Y(fake_Y), torch.ones(len(fake_Y)).to(device))
